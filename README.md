...
| **0:08–0:18** Teacher Modeling | 11-step guided tour with a spotlight: desktop, icons, App Launcher, system menu, single click vs. double click, scrolling, closing a window, returning to the desktop. Project it and have students mirror each step. |
| **0:18–0:38** Mouse & Keyboard Games | **Mouse Master** (pop = single click → select → double-click to open → right-click menu), **Scroll Quest** (wheel down for stars, then back up to the top), **Key Hunt** (letters, numbers, Space, Enter, Backspace, Shift, arrows → type your name → fix a word with Backspace) |
| **0:38–0:49** Scavenger Hunt / Closure | 7-item hunt (App Launcher, Files, system menu, open Paint, draw with a drag, close Paint, show the desktop), then the exit question and a printable certificate |

## Teacher Panel (👩‍🏫 button, bottom right)
